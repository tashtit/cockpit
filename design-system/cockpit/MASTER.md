...
- Keyframe animations are few and each means one thing: the 1.2s `pulse` dot while an agent is working (chat "working…" line + `LiveDot` on the board and in the palette), the 1s `spin` of the sidebar's `Spinner`, and a running roundtable's `rt-breathe`. A rotation of a fixed-size glyph moves nothing around it, so it is not the layout-shifting transform the anti-patterns ban.
...
- **`LiveDot`** (`.pulse.pulse-{agent}`) — 7px agent-colored pulse: "this session's agent is running right now". Occupies the row's exclusive meta slot (running beats landed beats PR badge beats timestamp) in board rows and palette rows, where the livery lighting up is the point.
- **`Spinner`** (`.spinner`) — the sidebar's running mark: an 11px ring (the `.asks-mark` glyph's size) in `--border-strong` with one `--fg` quarter, turning once a second. Same meaning and slot as `LiveDot`, but neutral: every sidebar row already leads with its agent's logo, and a rail of several agents at work lit up in three liveries at once. Holds still under reduced motion (the lit quarter still reads as "loading"). Always labelled (`Claude is working`, `round in progress`) unless its caller names it, as `.folded-news` does.
