...
- **`.landed-dot` / `.board-dot-landed`** — the same 7px in the agent's color, solid and still: "its turn ended and you haven't looked yet". Main owns the state (`attention-core.ts`) because it sees every turn end, including sessions no view has open, and the same list drives the Dock badge and desktop notifications; the renderer reports what the window shows (`setAttentionFocus`) so a watched session never lands, and `attention.ts` mirrors main's list for the rows. Always paired with a word (`landed <time>`, or an `aria-label`) — a dot alone would be colour carrying state.
- **`NeedMark`** (`.need-mark`) — the other "needs you" states, in the same slot: a 12px ring (10px in a sidebar row) with a glyph — **?** for an agent waiting on an answer or an approval, or a reviewer's change request; **!** for a failed turn or failing checks — in `--warn` or `--danger`. `role="img"` with the state in its name (`NEED_LABEL`: "asking a question — needs you"), and on the board the meta word says it again; the colour only echoes. `PrMark` (`.pr-mark`) is a PR number as a static placard for a board row that is itself the control — never a second button inside one.
- **`.board`** — the app's signature element (home only): departure-board of sessions — what needs you first (`.board-needs`, its own `needs you` placard label), then flying — livery-lit placard labels, branch chips, ticking elapsed time. Quiet `--surface` instrument panel; never give it the composer card's floating shadow. See `pages/home.md`.
