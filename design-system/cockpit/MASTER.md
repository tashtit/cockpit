...
- Narrow windows (≤780px) shed decorative chips before anything truncates; ≤700px the sidebar wordmark text sheds (the mark stays); short windows (≤600px) shed home's greeting and the masthead's sub-line, then the masthead's alarm scale; ≤559px — under the floor — the chat header's keys take a second line. Follow this "shed decoration first" pattern for new responsive cases. The shed media queries live at the **end** of style.css on purpose — earlier in the file they'd lose the cascade to same-specificity component rules; keep new shed rules there.
