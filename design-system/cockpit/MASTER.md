...
- **Mono:** `--mono` — **IBM Plex Mono** (same bundle, falling back to `ui-monospace`/`SF Mono`) carries two registers, keep them distinct:
  ...
  - *placards* (the instrument voice): the home board's masthead (`.board-mast`,
    the one place this voice is set at display scale), the `COCKPIT` wordmark, view
...
- Narrow windows (≤780px) shed decorative chips before anything truncates; ≤700px the sidebar wordmark text sheds (the mark stays); short windows (≤600px) shed the home masthead's sub-line and then its alarm scale; ≤559px — under the floor — the chat header's keys take a second line. Follow this "shed decoration first" pattern for new responsive cases. The shed media queries live at the **end** of style.css on purpose — earlier in the file they'd lose the cascade to same-specificity component rules; keep new shed rules there.
