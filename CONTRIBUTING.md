...
Merges land in bursts, so more than one push to `main` is often in flight. Each push gets its own concurrency group and is tested on its own — they used to share one, and since GitHub keeps only one run pending per group, the merges in the middle of a burst were evicted and never built. Packaging is what gets serialized instead: the `package` job takes a `main`-wide group where a newer push cancels an older build, so only the newest commit is packaged and released. A run overtaken that way ends by saying so, not by failing — semantic-release declines to publish a commit `main` has moved past (*"the local branch main is behind the remote one"*), and the run that overtook it releases every commit since the last tag, the overtaken one included.

A release is not one atomic step, and Cockpit splits it deliberately. semantic-release pushes the tag and writes the notes, but creates the GitHub Release as an empty draft and stops there (`draftRelease` in `.releaserc.json`); the `🎉 Publish the release` step then uploads the assets and publishes it. The plugin would happily do both, but it sends all nine assets at once with each file read whole into memory — roughly 540MB in flight from one runner — so the largest file stays exposed longest to GitHub's asset endpoint, which returns a 500 now and then. Three retries later it gives up with the tag already pushed, leaving an unreachable draft that no re-run can clear, because the next run sees the tag and decides no release is due. `v0.11.0` and `v0.13.0` were lost that way, both on the x64 disk image. `node scripts/publish-release.mts` uploads one file at a time, streamed off disk and retried on its own, and uploads only what is missing or was stored short — so re-running it repairs a half-filled release instead of starting over, and a release only goes public once every asset is on it.
