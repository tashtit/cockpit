...
- `src/shared/endpoints.ts` — custom model endpoints (BYOK): pure validation + env translation (`COPILOT_PROVIDER_*` for Copilot, `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN` for Claude; Codex unsupported — it would need config.toml writes). Definitions live in cockpit config; API keys never do — an endpoint names the env var holding its key, resolved at spawn. `chat.ts` injects the env per turn, and `index.ts` persists which endpoint each session started on (`sessionEndpoints`) so resumes stay on that backend and refuse loudly if the endpoint was removed.
