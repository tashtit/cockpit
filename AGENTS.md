...
- `github.ts` + `github-core.ts` — PR badges: one `gh pr list --json` per repo root, cached 60s, failing soft to an empty list. `-core.ts` is the IO-free half (what the unit tests target): it folds each PR's `statusCheckRollup` into `PrStatus.checks` (`passing | failing | pending | none` — any failure wins, then any unfinished or unrecognized run, else passing) and `reviewDecision` into `PrStatus.review`.
