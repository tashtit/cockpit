...
The lockfile was regenerated by a different npm major than the one installing it. `package.json` pins `packageManager` to the npm 10 that Node 22 bundles; npm 11 omits optional peer entries (the react 18 ones nested under `@docsearch/js`, a VitePress transitive) that npm 10 requires. Switch to the pinned toolchain and rewrite the lock:
...
The diff should be purely additive. Dependabot reads the same pin, so its PRs stay installable; if one fails this way, the pin and `.nvmrc` have drifted apart — CI's "npm matches the packageManager pin" step catches that on pull requests.
