...
import { describePlan, planFinish, type LocalAsset, type ReleaseState } from '../scripts/finish-release-core.mts'
...
describe('planFinish', () => {
  it('publishes a draft whose assets all landed', () => {
    expect(planFinish('0.11.0', release(), dist)).toEqual({ action: 'finish', upload: [], publish: true })
  })

  it('re-uploads the asset the 500 lost, then publishes — the v0.11.0 failure', () => {
    const assets = release().assets.filter((a) => a.name !== 'Cockpit-0.11.0-x64.dmg')
    expect(planFinish('0.11.0', release({ assets }), dist)).toEqual({
      action: 'finish',
  ...
  it('re-uploads an asset GitHub never finished storing', () => {
    ...
    expect(planFinish('0.11.0', release({ assets }), dist)).toMatchObject({ upload: ['latest-mac.yml'] })
  })
  ...
  it('re-uploads an asset stored short, which would otherwise look like a good download', () => {
    ...
    expect(planFinish('0.11.0', release({ assets }), dist)).toMatchObject({
      upload: ['Cockpit-0.11.0-x64.dmg']
    })
  })

  it('does nothing for a release that is already published and complete', () => {
    expect(planFinish('0.11.0', release({ isDraft: false }), dist)).toEqual({ action: 'none' })
  })

  it('still repairs a published release that is missing an asset', () => {
    const assets = release({ isDraft: false }).assets.slice(1)
    expect(planFinish('0.11.0', release({ isDraft: false, assets }), dist)).toEqual({
      action: 'finish',
  ...
  it('refuses when semantic-release never got as far as creating the release', () => {
    expect(planFinish('0.11.0', null, dist)).toMatchObject({ action: 'refuse' })
  })

  it('refuses a release tagged something other than the packaged version', () => {
    expect(planFinish('0.11.0', release({ tagName: 'v0.10.0' }), dist)).toMatchObject({ action: 'refuse' })
  })

  it('refuses when there is no packaged version or nothing was built', () => {
    expect(planFinish('', release(), dist)).toMatchObject({ action: 'refuse' })
    expect(planFinish('0.11.0', release(), [])).toMatchObject({ action: 'refuse' })
  })
})
...
describe('describePlan', () => {
  it('says what it is about to do', () => {
    expect(describePlan('0.11.0', { action: 'finish', upload: ['a.dmg'], publish: true })).toBe(
      'finishing v0.11.0: re-uploading a.dmg; publishing the draft'
    )
    expect(describePlan('0.11.0', { action: 'none' })).toContain('already published')
    expect(describePlan('0.11.0', { action: 'refuse', reason: 'nope' })).toBe('cannot finish v0.11.0: nope')
  })
})
