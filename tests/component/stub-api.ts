...
export function freshApi(): CockpitApi {
  return {
    ...
    getAttentionPrefs: vi.fn(async () => ({ notifications: true, sound: true, badge: true })),
    ...
    stopProcesses: vi.fn(async () => ({ cleaned: 0, freedBytes: 0, failed: [] })),
    getPrs: vi.fn(async () => []),
    ...
    setZoomFactor: vi.fn(),
    reportZoom: vi.fn(async (factor: number) => factor),
