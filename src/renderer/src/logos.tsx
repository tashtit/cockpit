...
/* Trash: the cleanup view's mark — what this place is for is throwing things away. */
const OCTICON_TRASH =
...
export const TrashIcon = ({ size = 16 }: { size?: number }): JSX.Element => (
  <Octicon d={OCTICON_TRASH} size={size} />
)
...
/** octicon arrow-down — "there is more below" (the transcript's jump-to-latest key) */
const OCTICON_ARROW_DOWN =
  'M13.03 8.72a.75.75 0 0 1 0 1.06l-4.5 4.5a.75.75 0 0 1-1.06 0l-4.5-4.5a.75.75 0 0 1 1.06-1.06l3.22 3.22V2.75a.75.75 0 0 1 1.5 0v9.19l3.22-3.22a.75.75 0 0 1 1.06 0Z'
export function ArrowDownIcon({ size = 12 }: { size?: number }): JSX.Element {
  return <Octicon d={OCTICON_ARROW_DOWN} size={size} />
}
/** octicon download — a newer Cockpit, on its way or waiting (the footer's update bar) */
const OCTICON_DOWNLOAD =
...
export const UpdateIcon = ({ size = 12 }: { size?: number }): JSX.Element => (
  <Octicon d={OCTICON_DOWNLOAD} size={size} />
)
