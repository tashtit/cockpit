...
// Chromium's notices ride with Electron's binary, which Electron ≥44 only unpacks the first
// time something requires the package — requiring it here unpacks it (from the download
// cache when warm). electron-builder merely warns about a missing extraResources source,
// so a missing file fails here instead of shipping an app without it.
require('electron')
const chromiumNotices = path.join(path.dirname(require.resolve('electron/package.json')), 'dist', 'LICENSES.chromium.html')
if (!existsSync(chromiumNotices)) throw new Error(`Chromium's notices are missing at ${chromiumNotices}`)
...
module.exports = {
  ...
  npmRebuild: false,
  asar: true,
  // Outside the asar so About can open them in a text viewer. The notices are generated by
  // `npm run build` (scripts/licenses); Chromium's come with Electron's binary, which a mac
  // build does not otherwise carry into the bundle.
  extraResources: [
    { from: 'out/THIRD_PARTY_NOTICES.txt', to: 'THIRD_PARTY_NOTICES.txt' },
    { from: chromiumNotices, to: 'LICENSES.chromium.html' }
  ],
